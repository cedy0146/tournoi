...
public class TournoiDAO {
    ...
    public void insert(Tournoi t) throws SQLException {
        String sql = "INSERT INTO tournoi (nom, type, date_debut, date_fin, statut) VALUES (?, ?, ?, ?, ?)"; // id_tournoi est auto-incrémenté
        try (Connection conn = DBConnection.getConnection();
            ...
            if (affectedRows > 0) {
                try (ResultSet keys = ps.getGeneratedKeys()) {
                    if (keys.next()) t.setId_tournoi(keys.getInt(1));
                }
            }
        }
    }
    ...
    private Tournoi mapRow(ResultSet rs) throws SQLException {
        ...
        if (dDebut != null) t.setDateDebut(new java.util.Date(dDebut.getTime()));
        
        Date dFin = rs.getDate("date_fin");
        if (dFin != null) t.setDateFin(new java.util.Date(dFin.getTime()));
